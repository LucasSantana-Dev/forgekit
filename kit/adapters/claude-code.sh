...
adapter_status() {
  ...
  if [ -d "$claude_dir/skills" ]; then
    skills_count=$(find "$claude_dir/skills" -maxdepth 1 -name "*.md" 2>/dev/null | wc -l)
  fi
...
adapter_uninstall() {
  ...
  if [ -d "$claude_dir/skills" ]; then
    if [ -f "$claude_dir/skills/.forge-kit" ] || grep -q "^# forge-kit" "$claude_dir/skills"/*.md 2>/dev/null | head -1; then
      if [ "${FORGE_DRY_RUN:-false}" = "true" ]; then
