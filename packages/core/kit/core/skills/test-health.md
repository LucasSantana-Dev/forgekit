...
- Before running `/test-cleanup`, `/mutation-test`, `/generate-tests`, or
  `/flaky-test-hunter` to confirm which one is actually needed
...
A full flake hunt belongs in `/flaky-test-hunter`. For a quick estimate, run the suite
3 times and compare:
...
- **Flaky in 3 runs** → FLAKY → `/flaky-test-hunter`
